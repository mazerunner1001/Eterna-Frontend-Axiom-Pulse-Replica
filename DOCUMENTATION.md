...
1. **Mock Data**: No real API integration. All data generated client-side.
2. **WebSocket**: Simulated with `setInterval`, not real WebSocket connection.
3. **Persistence**: No database. Data resets on page reload.
4. **Authentication**: No user accounts or authentication system.
5. **Server-Side Rendering**: Initial render is client-side only for dynamic data.

---

## Future Enhancements

1. **Real API Integration**: Replace mock data with actual blockchain data
2. **WebSocket Connection**: Connect to live price feeds
3. **User Accounts**: Save preferences and watchlists
4. **Advanced Filtering**: Multi-criteria filters with persistence
5. **Chart Integration**: Add TradingView charts
6. **Notifications**: Price alerts and notifications
...
**Cause**: Token ID collision during infinite scroll
**Solution**: Verify statusOffsets in `src/lib/api.ts` are unique

### Issue: Performance Degradation
**Symptom**: Slow scrolling after loading many tokens
**Cause**: Too many DOM nodes or missing memoization
**Solution**: 
1. Check React.memo on TokenCard
2. Verify useCallback on scroll handlers
3. Consider virtual scrolling for >300 tokens

### Issue: Price Updates Not Showing
**Symptom**: No flash animations on price changes
**Cause**: WebSocket mock not running or Redux not connected
**Solution**:
1. Check `webSocketEnabled` state in page.tsx
2. Verify Redux DevTools shows updatePrice actions
3. Check priceUpdate selector in TokenCard

### Issue: Modal Not Closing
**Symptom**: Filter modal stays open after clicking outside
**Cause**: Radix Dialog portal not properly mounted
**Solution**: Verify Modal component has correct Radix imports
